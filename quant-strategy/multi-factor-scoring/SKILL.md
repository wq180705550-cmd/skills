...
triggers:
  ...
  - "回测稳健性评级/MinervaScore/后选择过拟合/DSR+PBO+SPA+MTR"
  - "增长最优组合/KellyBoost/梯度提升树 Kelly/精确增长损失"
  - "环路增益矩阵/再平衡反馈/谱半径稳定性盲区/权杆ETF复杂"
  - "广义二次风险/Markowitz推广/切点≠最大Sharpe/SPD风险矩阵"
  - "期权隐含堵盘/IV伞差/风险中性偏度/regime依赖可预测性"
  - "系统性风险传染/时序多重图神经网络/银行CDS/边扰动传导"
...
## 13.14 This-Week arXiv Integration (2026-08-24 ~ 2026-08-30)

Crawled the arXiv **q-fin** `pastweek` listing (announcements 2026-08-24 → 2026-08-28, ~60+ papers across q-fin.{CP,PM,ST,RM,MF,TR} + cross-lists from cs.LG/cs.AI/stat.ML/cond-mat/econ) plus the per-category pastweek pages to recover the Tue-25 overflow; scanned all titles, read 9 abstracts, and selected 9 papers grouped into 9 subsections with a direct, non-speculative mapping. Follows the opt-in, config-driven, graceful-fallback pattern of §13.6–§13.13. Two of the nine (§13.14.5, §13.14.9) are re-announced this week via cross-list/replacement though originally submitted earlier in 2026; both are highly relevant and kept, with original dates noted.

This week's theme is **backtest evidence-discipline upgrade + growth-optimal / generalized-risk allocation + rebalancing & systemic-stability monitoring + spectral/coherence calibration guards**: MinervaScore turns the backtest audit from a checklist into a *quantitative post-selection robustness grade*; KellyBoost and quadratic-risk Markowitz upgrade the *sizing engine*; the loop-gain matrix and systemic-risk GNN upgrade *rebalancing & contagion monitoring*; option-implied crash signals and cross-regime BO refine *regime-dependent signal gates*; and the temporally-correlated random-matrix result plus the same-day directional benchmark are *calibration/skepticism guards* (the latter a negative-leaning benchmark).

#### §13.14.1 MinervaScore: 回测后选择稳健性评级 (arXiv:2608.23808)

**Paper:** "Equity Strategy Backtesting: Luck or Edge? The MinervaScore as a Statistical Robustness Grade" (Santoni, Jouanne, Scullin; submitted 2026-08-24)

**Method:** A post-selection robustness grade that combines four established validation quantities — Deflated Sharpe Ratio (DSR), Probability of Backtest Overfitting (PBO), Superior Predictive Ability (SPA), Minimum Track Record Length (MTR) — with a regime-stability diagnostic. Each is converted to a signed margin from its admissibility threshold, aggregated into a raw score, mapped to a 0–100 display tied to a binary Robustness Seal (≥ 80 only when all five gates pass). Calibrated on 359,062 production backtest records. AUROC 0.989 vs true signal in synthetic markets with known ground truth; but a pre-registered real-market test showed **no significant forward relationship** (Spearman ρ_s = 0.013, p = 0.40) — the score ranks statistical support, not the probability of future profit.

**Framework mapping:** Upgrades **§13.12.4 spec-satisfaction backtest verification** and **`audit_checklist()` (#25)**, and pairs with **§13.10.4 backtest audit**. Moves the backtest gate from a *pass/fail checklist* to a *continuous, multi-component robustness grade* with an explicit Robustness Seal — directly attacking the "search luck vs genuine edge" failure mode (#25 already flags purge/embargo/point-in-time; MinervaScore adds the post-selection multiplicity accounting).

**Signal design (opt-in, default OFF; recommended ON as a pre-promotion gate):**
- `minervascore(backtest_record)` → {DSR, PBO, SPA, MTR, regime_stability} margins → 0–100; returns `robustness_seal` (True only if score ≥ 80 AND all 5 gates pass). Wire as a **mandatory pre-promotion gate** before any strategy leaves `shadow_before_swap` (#32) / goes live.
- **Admission:** requires the backtest to already expose candidate-count, OOS validation, and history length (the three quantities standard summaries omit). Graceful fallback to `audit_checklist()` (#25) when components unavailable.
- **Caveat (evidence discipline):** the score **does not predict future profit** (real-market pre-registered ρ_s = 0.013, p = 0.40). It is an *auditing/reporting layer*, not alpha — and must never be read as "this strategy will make money." Pairs with #25; strengthens the §13.13.6 calibration-viability spirit (acquire/trust only what survives evidence).

#### §13.14.2 KellyBoost: 增长最优梯度提升树组合 (arXiv:2608.23393)

**Paper:** "KellyBoost: Growth-Optimal Portfolio Construction with Gradient-Boosted Trees" (Jiayu Li; submitted 2026-08-24)

**Method:** A single multi-output XGBoost whose softmax output *is* the portfolio: with y the vector of per-asset holding-period returns, the training loss is −log(1 + w·y), the negative log growth rate, so the fitted model is the growth-optimal (Kelly) allocation conditioned on the features. The objective is **exact** (not a surrogate): closed-form gradient, analytic diagonal Hessian, and full Hessian, all finite-difference verified; ships a dependency-free reference engine.

**Framework mapping:** Upgrades **§13.11.1 conformal-Kelly sizing**, **§5 position sizing**, and **§15.4 ML-portfolio layer**. Bridges *Kelly growth-optimality* with a *tree-ensemble feature map* — the portfolio becomes a directly-trained growth-optimal function of features, refining the fractional-Kelly sizing of §13.11.1 with a learnable, feature-conditioned allocation.

**Signal design (opt-in, default OFF):**
- `kellyboost_alloc(features, returns)` → multi-output XGBoost softmax weights maximizing the exact growth rate −log(1+w·y); use as a feature-rich drop-in for §13.11.1 sizing when feature coverage is rich.
- **Admission:** requires feature matrix + per-asset holding-period returns; apply **fractional Kelly** (kelly_fraction ≤ 0.5, per #38) — full-Kelly is aggressive; graceful fallback to §13.11.1 conformal-Kelly when features thin. Must clear walk-forward (#25) + ESS gate (#33); A-share features must be point-in-time (§13.10.7).
- **Caveat:** tree ensembles can overfit (#37/#43); the exact-growth objective does not by itself guarantee OOS growth — require `audit_checklist()` (#25) + ESS (#33) before trusting. Consistent with #38's "calibration ≠ growth" warning.

#### §13.14.3 The Loop-Gain Matrix: 耦合再平衡反馈的稳定性盲区 (arXiv:2608.22768)

**Paper:** "The Loop-Gain Matrix: Coupled Rebalancing Feedback and the Blind Spots of Scalar Stability Monitoring" (Jihwan Woo; submitted 2026-08-24)

**Method:** Leveraged-ETF complex stability is governed by the **spectral radius ρ(L)** of a loop-gain matrix L, not by any single product's scalar loop gain — scalar per-product monitoring *underestimates* system feedback by construction. Two blind spots: (i) cycle amplification (ρ(L) ≥ max_i l_ii for nonnegative coupling, strict under two-way coupling); (ii) transmitted displacement (arises under one-way coupling, invisible to the receiver's own gain). A reduced-form estimator of L needs only prices + public fund assets (via cross-asset overnight reversals). In the 2026 Korean single-stock LETF episode it detects transmission from the SK Hynix complex into Samsung's close (DiD z = −2.82; p = 0.0055), with ~41% of Samsung's closing-displacement variance imported — invisible to its own "moderate" scalar gain of 0.24.

**Framework mapping:** Upgrades **§13.10.3 shadow-before-swap / rebalancing discipline**, **§13.13.5 state-dependent reallocation**, and **§13.10.6 market-impact**. Reframes rebalancing-risk monitoring from *per-product scalar gain* to a *coupled (complex × venue) matrix spectral radius* — directly exposing the feedback blind spots that scalar monitoring misses.

**Signal design (opt-in, default OFF; monitoring state):**
- `loop_gain_matrix(complex_prices, fund_assets)` → estimate L via cross-asset overnight reversals; compute ρ(L); elevated ρ(L) vs scalar gains ⇒ flag *systemic rebalancing-feedback risk* ⇒ tighten rebalancing cadence / widen impact buffer (§13.10.6) / lower `risk_scale`.
- **Admission:** requires ≥ 2 correlated underlyings with public fund AUM (LETF/ETN complexes, A-share leveraged/futures complexes); graceful fallback to per-product loop-gain when matrix inestimable. Estimator is convention-sensitive — report measurement-convention sensitivity.
- **Caveat:** a *monitoring/diagnostic state*, not a directional signal. Organize monitoring around the (complex × venue) matrix, not products (#43/#18 discipline). Pairs with §13.12.5 inter-sectoral imbalance.

#### §13.14.4 Generalizing Markowitz by a Quadratic Risk Measure: 广义二次风险配置 (arXiv:2608.24449)

**Paper:** "Generalizing Markowitz Portfolio Optimization by a Quadratic Risk Measure" (Gasparavičius, Grigutis; submitted 2026-08-25)

**Method:** The key closed-form results of classical Markowitz (variance risk) remain available under a broader class of **strictly convex quadratic risk measures**: replace the covariance matrix with an arbitrary symmetric positive-definite matrix and allow additional linear + constant terms. Closed forms are derived for the efficient frontier, global-minimum-risk portfolio, maximum-Sharpe portfolio, Capital Market Curve, tangency portfolio, and maximum-utility portfolio. A new geometric phenomenon: under the generalized measure the **tangency portfolio no longer coincides with the maximum-Sharpe portfolio**.

**Framework mapping:** Upgrades **§5 position sizing**, **§13.9.4 CVaR allocation**, and **§13.11.4 Wasserstein DRO**. Supplies a *drop-in generalized quadratic-risk solver* covering transaction-cost penalties, benchmark-relative optimization, covariance regularization, and factor models — all within one closed-form family, refining the mean-variance core of §5.

**Signal design (opt-in, default OFF):**
- `quadratic_risk_portfolio(R, lin, const, mu)` → closed-form weights for efficient frontier / tangency / max-Sharpe under generalized quadratic risk R (e.g. shrunk/regularized covariance, transaction-cost quadratic penalty, factor-model risk). Use as a replacement for the §5 mean-variance solver.
- **Admission:** requires SPD risk matrix R; graceful fallback to classical Markowitz (variance) when R reduces to Σ. Validate OOS vs §13.11.4 Wasserstein DRO.
- **Caveat:** stays within the *quadratic-risk class* (not for CVaR/DRO explicitly); because tangency ≠ max-Sharpe under generalization, the objective must be chosen **explicitly** (do not assume the classical coincidence). Theoretical; pair with #25/#33 before live sizing.

#### §13.14.5 Option-Implied Signals and Crash Risk: 区间依赖的崩盘信号 (arXiv:2608.26115)

**Paper:** "Option-Implied Signals and Crash Risk: Predictability and Machine-Learning Evidence from U.S. Equity Options" (Li, Wang; submitted 2026-06-10, re-announced in q-fin.ST pastweek 2026-08-28)

**Method:** Re-estimates canonical option-implied predictability on a unified 2015–2026 panel of 12.36M U.S. equity firm-day observations across 10,026 underlyings, split into three regimes (late-post-crisis low-vol 2015–2019; high-vol transition 2020–2022; AI/mega-cap concentration 2023–2026). The Xing et al. (2010) smirk–return relation **decays**: next-month univariate smirk coefficient falls from −0.023 (t = −5.5) in 2015–2019 to an insignificant −0.006 (t = −1.5) in 2023–2026, turning positive at the 3-month horizon. By contrast, the Cremers–Weinbaum IV spread and a Bakshi-style risk-neutral skewness stay significant across regimes and specs. A boosted tree beats linear models for next-month return only in the AI/mega-cap regime (R²OOS +1.29 vs +0.07); for 5-day crash classification AUC is highest in calm markets (0.706) and lowest in the high-vol transition (0.561).

**Framework mapping:** Upgrades **§13.8.5 tail / CVaR gate** and **§13.13.4 cross-asset rough-vol**. Supplies *regime-dependent option-implied crash inputs*: the IV spread and risk-neutral skewness are robust across regimes (use them), while the canonical smirk is regime-decayed (do not use stale coefficients) — refining the tail-risk gate with regime-conditioned signal selection.

**Signal design (opt-in, default OFF; feeds §13.8.5 tail gate):**
- `option_implied_crash_gate(iv_spread, rn_skew, regime)` → use Cremers–Weinbaum IV spread + RN skewness (NOT the weakening smirk) as crash-risk inputs to §13.8.5 tail gate; regime-switch the model (linear vs tree by regime, tree only where edge exists).
- **Admission:** requires option-chain IV spread + RN-skew series (A-share options thin → graceful fallback to §13.8.5 parametric tail). 5-day crash AUC only 0.561 in high-vol — weak; treat as a *risk modulator*, not a directional signal.
- **Caveat (evidence discipline):** the smirk signal is **regime-decayed** (insignificant post-2023) — never ship stale smirk coefficients; the boosted-tree edge is regime-localized (AI/mega-cap only). Pairs with #18/#31 — option-implied predictability is real but regime-dependent.

#### §13.14.6 Systemic Risk Transmission: 时序多重图神经网络 (arXiv:2608.27295)

**Paper:** "A Temporal Multiplex Graph Neural Network for Systemic Risk Transmission in Global Banking" (Umeorah, Fadina; submitted 2026-08-27)

**Method:** A Temporal Heterogeneous Multiplex GNN assesses systemic risk and contagion channels in global banking. Builds a harmonized quarterly panel (bank fundamentals + CDS spreads + macro) represented as dynamic multiplex networks linking banks via financial similarity and liquidity co-movement, augmented with country-level macro relationships. Integrates graph convolution with recurrent GRU dynamics and a learnable fusion gate capturing time-varying reliance on alternative contagion channels. Outperforms econometric/ML/graph benchmarks for short-term CDS-spread changes; provides interpretable bank-level systemic importance (stress testing), country-level spillovers, and transmission pathways via edge-perturbation analysis.

**Framework mapping:** Upgrades **§13.12.5 inter-sectoral signed-network imbalance** and **§13.13.3 representation crowding**. Supplies a *temporal multiplex network* view of contagion with interpretable transmission pathways (edge perturbation) — a richer systemic-risk overlay than the static signed-network imbalance of §13.12.5.

**Signal design (opt-in, default OFF; macro/systemic overlay):**
- `systemic_risk_read(multiplex_net)` → bank-level systemic importance + country spillover + transmission pathways; feed as a *macro/systemic-risk overlay* to `risk_scale` / regime (§13.3). Edge-perturbation analysis → identify contagion channels to monitor.
- **Admission:** requires bank fundamentals + CDS + macro panel (A-share: needs domestic interbank/CDS proxies; quarterly frequency = slow). Graceful fallback to §13.12.5 intersectoral imbalance index.
- **Caveat:** trained on global banking CDS; A-share application needs domestic proxies and is low-frequency. Diagnostic/monitoring, not alpha. Pairs §13.12.5.

#### §13.14.7 Cross-Regime Bayesian Optimisation for Signal Generation: 跨regime稳健信号搜索 (arXiv:2608.27076)

**Paper:** "Tabular Deep Learning for Algorithmic Trading: Cross-Regime Bayesian Optimisation for Equity Signal Generation" (Le Grice; submitted 2026-08-27)

**Method:** Five model classes trained on ~300 large-cap US equities over 11 years, with Bayesian optimisation configured to target trading performance **across three statistically different regimes** (not just in-sample fit). Regime-robust hyperparameter selection yields OOS generalisation: signal precision stays above the random baseline across all four test quarters; portfolio performance degrades slowly under simulated input noise before collapsing beyond a threshold. No single tabular architecture beats gradient-boosted trees, but XGBoost + TabNet rank aggregation (Hybrid) reaches annualised return 51.26%, Sharpe 2.44, CAPM alpha 0.423 (p = 0.011, near-zero beta → stock-selection driven). Alternative data plays a secondary role once technical/fundamental features are in.

**Framework mapping:** Upgrades **§15.3 online-learning guard**, **§13.12.1 regime-gated volatility routing**, and **§13.10.3 shadow-before-swap**. Supplies a *regime-robust hyperparameter-selection protocol*: the BO objective is cross-regime OOS trading performance, directly attacking the #46 scope-boundary concern (HP tuned on one regime fails in others).

**Signal design (opt-in, default OFF):**
- `regime_robust_hp_search(model, regime_splits)` → BO objective = cross-regime OOS trading performance (not accuracy); select HP keeping signal above random baseline in **all** regimes; rank-aggregate XGBoost + TabNet as the Hybrid ensemble.
- **Admission:** requires pre-labeled regime splits (use §13.12.1 router or external); graceful fallback to single-regime BO when regimes unlabeled. Must clear walk-forward (#25) + ESS (#33); the 51% return is a research result on 300 large-caps (survivorship/look-ahead risk) — require §13.12.4 before trusting.
- **Caveat:** alternative data is secondary once tech/fundamental in (#18); the Hybrid's headline return needs walk-forward + ESS before any live use. Pairs #46/#43.

#### §13.14.8 Temporally Correlated Random Matrices: 谱校准护栏 (arXiv:2608.23944)

**Paper:** "Bulk Phase Transition and Edge Behavior in Temporally Correlated Random Matrices" (Hisakado, Kaneko; submitted 2026-08-25)

**Method:** Studies long-range-correlated Wigner-type matrices from row-independent stationary Gaussian sequences. For exponentially decaying (AR(1)) correlations the bulk spectral density deforms from the semicircle law via an explicit "hub" mechanism, with Tracy–Widom edge universality holding for every fixed ρ < 1. For power-law correlations d_t ∼ t^−γ, γ_c = 1/2 is the critical point for divergence of the bulk fourth-moment, while γ = 1 marks breakdown of the flatness condition governing the MDE edge analysis. The fourth-moment transition is proved exactly; the self-consistent edge varies smoothly across γ = 1.

**Framework mapping:** Upgrades **§13.11.2 exposure-similarity graph** and **§13.12.6 lower-spectrum synchronization factor**. Supplies a *temporal-correlation correction* to the random-matrix spectral assumptions underlying exposure-graph eigen-analysis and lower-spectrum factors: strong temporal correlation (γ < 1/2) pushes the bulk/edge away from the iid Marchenko–Pastur reference, so eigenvalue shrinkage and lower-spectrum calibration must be adjusted.

**Signal design (guard, opt-in, default OFF; spectral calibration):**
- `correlation_matrix_spectral_guard(corr, temporal_gamma)` → estimate the temporal-correlation exponent γ of the empirical correlation matrix; if γ < 1/2 (strong long-range correlation) ⇒ bulk/edge deviate from iid reference ⇒ adjust eigenvalue-shrinkage / lower-spectrum factor (§13.12.6) calibration and flag non-iid contamination.
- **Admission:** applies to any spectral method on empirical correlation matrices (§13.11.2 / §13.12.6). Pure theory on Wigner-type matrices — translate to financial correlation matrices with care.
- **Caveat:** a *calibration guard* for spectral methods, not a signal. Directly enforces the #16 no-IID-assumption rule (and #11/#17 dependency-aware sampling) at the eigenstructure level. Pairs with §13.12.6.

#### §13.14.9 [GUARD] Same-Day Directional Benchmark: 同日方向预测的证据纪律 (arXiv:2608.26106)

**Paper:** "A Statistical-Finance Benchmark for Same-Day Directional Stock Prediction: Walk-Forward Evidence from SPY" (Alex Chen; submitted 2026-05-01, re-announced in q-fin.ST pastweek 2026-08-28)

**Method:** Benchmarks XGBoost / RF / LightGBM / LogReg under expanding-window walk-forward on SPY (1993–2024) predicting whether the same day's close beats the prior close from open + two lagged prices. Logistic Regression attains the highest close-direction accuracy 71.09% on the last 800 days; XGBoost 58.45% (95% CI [54.94%, 62.08]); XGBoost accuracy rises to 72.7% only when the predicted move exceeds 1%, but the usable sample collapses to 154 observations. Concludes: simple daily features contain *detectable* same-day directional information, but the result needs careful sample-size accounting and supports only *limited* economic claims.

**Framework mapping:** A strong **guard** on **§13.11.6 sentiment-classification ≠ return**, **§13.12.8 classification-boundary proximity**, and **#14 base-rate honesty**. Re-enforces *directional-signal skepticism*: a 71% accuracy number is meaningless without sample-size accounting and bootstrap CI, and the economically usable edge shrinks sharply once a meaningful threshold is imposed.

**Signal design (guard, opt-in, recommended ON for all directional classifiers):**
- `same_day_directional_guard()` → *calibrate expectations*: any directional signal's accuracy must be reported with (a) bootstrap CI, (b) sample-size accounting, (c) the usable-sample size at the trading threshold; reject strategies whose edge vanishes under proper sample-size accounting. Mirror #14 (base-rate-honest CI) at the *evaluation* stage.
- **Admission:** applies to **all** directional/classification signals (§13.11.6, §13.12.8, #14, #43). Uses walk-forward + bootstrap, no holdout leakage.
- **Caveat:** this is a **benchmark/guard**, not alpha. 71% accuracy is on a narrow task (SPY same-day close vs prior close) with an economically limited edge; do not read it as "daily directional prediction works." Pairs with §13.11.6 and #25/#33 — directly strengthens the "directional signal skepticism" discipline.
...
63. **强制**：任何策略**晋升/上线前**须过 `minervascore` 后选择稳健性评级（DSR + PBO + SPA + MTR + regime 稳定性五闸合取，≥ 80 且全闸通过才发 Robustness Seal）；评分**不预测未来收益**（真实市场预注册 ρ_s = 0.013, p = 0.40），仅作审计/报告层，禁止读作"会赚钱"；与 #25 审计、§13.13.6 校准可行性一致。
64. **推荐**：增长最优配置可用**特征条件 KellyBoost**（softmax = XGBoost 组合、损失 −log(1+w·y) 精确增长目标）替代/增强 §13.11.1 共形 Kelly；必须**分数 Kelly**（≤ 0.5）且过 walk-forward(#25) + ESS(#33)，A 股特征须 point-in-time(§13.10.7)；树集成过拟合风险呼应 #37/#43。
65. **强制**：杠杆/再平衡产品稳定性监控须用**环路增益矩阵谱半径 ρ(L)**而非单产品标量增益（标量监控结构性低估系统反馈，盲区：环放大 + 传导位移）；监控须围绕 (complex × venue) 矩阵组织，ρ(L) 升高 ⇒ 收紧再平衡节奏 / 抬升冲击缓冲(§13.10.6) / 降 risk_scale；仅监控态非方向信号，与 #43/#18 一致。
66. **推荐**：均值方差求解可推广为**广义二次风险**（任意 SPD 风险矩阵 + 线性/常数项，封闭解覆盖有效前沿/切点/最大 Sharpe/最大效用）；注意广义下**切点组合 ≠ 最大 Sharpe**，目标须显式选定，不得套用经典重合；须过 #25/#33 再实盘。
67. **强制**：期权隐含崩盘信号须**按 regime 选择**：Cremers–Weinbaum IV 价差 + 风险中性偏度跨 regime 稳健（用作 §13.8.5 尾部闸门输入），**经典 smirk 系数已 regime 衰减**（2023+ 不显著）禁止装运陈旧 smirk 系数；boosted-tree 边缘仅 AI/mega-cap regime 局部（R²OOS +1.29），5 日崩盘 AUC 高波动仅 0.561（弱），作风险调制非方向信号；与 #18/#31 一致。
68. **推荐**：系统性风险监控可补**时序多重图神经网络**传染通道视图（银行基本面 + CDS + 宏观动态多重网络，边扰动剖析传导路径），作为宏观/系统风险 overlay 喂 risk_scale / regime(§13.3)；A 股需国内银行间/CDS 代理且季度低频，诊断/监控非 alpha，优雅回退 §13.12.5。
69. **强制**：超参/模型选择的目标须为**跨 regime OOS 交易表现**而非 in-sample 拟合（贝叶斯优化按三 regime 联合目标），确保信号在所有 regime 均高于随机基线才采纳；单一 regime 调参在别 regime 失效（#46 范围边界），须过 walk-forward(#25) + ESS(#33)，Hybrid 51% 收益为研究值（生存/前视风险）禁止直接照搬。

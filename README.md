...
| [multi-factor-scoring](./quant-strategy/multi-factor-scoring/) | 多因子量化交易系统：6-Category 因子选股 + 4-Layer 趋势萌芽框架（OI/ATR/OBV/CMF/Supertrend/HMA/Donchian/DMI/MACD）+ 否决项规则 + FTS 因子治理（三级评估链/走航验证/衰减检验/熔断/正交化/原子持久化）+ 部署纪律层（ESS 有效样本量闸门/Shadow-Before-Swap 换模门控/区间相干性投影/被动市场冲击成本/羊群拥挤度）+ 不确定性→仓位&稳健结构层（共形Kelly区间宽度仓位/暴露相似度因子图MINGLE/非高斯长记忆回撤预算/认证Wasserstein分布鲁棒配置/预测差Shapley归因/情绪分类≠收益护栏/板块嵌入横截面异质性）+ 实盘反馈&范围边界层（自动重校准触发器/在线学习护栏/实时成本实证标定/ML组合层范围边界/另类数据准入边界）+ 本周arXiv集成层（regime路由波动率/校准期覆盖部署闸门/解耦alpha-beta触发/规范满足回测验证/部门间失衡监控/下谱同步因子/FOMC预公告闸门/分类边界邻近监控、§13.13 可解释Itô签名/共动重构速率/表征拥挤/跨资产粗糙vol/CVaR状态依赖非对称/LLM校准可行性检查点/M3微观结构仿真/稳定RL辅助任务、§13.14 MinervaScore回测评级/KellyBoost增长最优树/环路增益矩阵再平衡稳定/广义二次风险Markowitz/期权隐含崩盘regime闸门/系统性风险多重图神经网络/跨regime贝叶斯信号搜索/时序相关随机矩阵谱护栏/同日方向基准护栏、§13.15 agentic量化工作流综述/熵因子模型稳健复制/小盘含权神经协方差收缩/订单流长记忆指数/VRP卖Put去风险/披露beta regime条件LLM/诚实评估结构化无前视+搜索deflation护栏/IlliQaR流动性尾部、§13.16 订单流regime变点检测BOCPD/ALM-GARCH非对称长记忆方差/ESFM共同损失严重度因子/信号相关性vs PnL相关性非可识别性/椭圆最优二元比例差区间EO CI/市场信息联合极值网络JEAM/regime切换稳健深度RL做市/EVaR风险平价）+ 观点类信号证据验证契约层（§16 signal-attribution：可证伪record→独立行情源事后label→Wilson CI基率诚实→独立维度归因→样本≥30准入） | v2.14.0 |
